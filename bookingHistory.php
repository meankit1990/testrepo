...
function getBookingList($userId) {
	$bookingListQuery = "SELECT * from ridesavailable where userId = '$userId'";
	$res = mysql_query ( $bookingListQuery );
	$ridesArray = array ();
	if (mysql_num_rows ( $res ) > 0) {
		$i = 0;
		$query = "SELECT DISTINCT bookings.*,bookings.id as requestId,ridesavailable.*,ridesavailable.id as ride_id, users.*,users.user_id as driverId,routes.routeId as routeId,cars.*,cars.id as carId  FROM bookings,ridesavailable,users,routes,cars WHERE (ridesavailable.userId = '$userId' and rideId = ridesavailable.id) and (users.user_id = bookings.userId) and (routes.routeId = ridesavailable.id) and cars.id = ridesavailable.carId";
